...
export class ProviderBranchService {
  ...
  async create(
    providerId: number,
    dto: CreateProviderBranchDto,
  ): Promise<ProviderBranchEntity> {
  ...
  async update(
    id: string,
    dto: UpdateProviderBranchDto,
  ): Promise<ProviderBranchEntity> {
    return this.dataSource.transaction(async (tx) => {
      const repo = tx.getRepository(ProviderBranchEntity);
      const existing = await repo.findOne({ where: { id } });
      if (!existing)
        throw new NotFoundException(`ProviderBranch ${id} not found`);
  ...
  private async demoteOtherMainBranches(
    tx: {
      getRepository: (
        e: typeof ProviderBranchEntity,
      ) => Repository<ProviderBranchEntity>;
    },
    providerId: number,
    exceptId?: string,
  ): Promise<void> {
    const qb = tx
      ...
      .where('provider_id = :providerId AND is_main_branch = TRUE', {
        providerId,
      });
