...
export class ShippingQuoteService {
   ...
   */
  async quoteForOrder(orderId: string, override?: QuoteOverride): Promise<QuoteResult> {
    this.assertEnabled();
    ...
    const destination: SkydropxAddress = {
      ...
      company: override?.destination?.company ?? order.shipToCompany ?? undefined,
      street1: override?.destination?.street1 ?? order.shipToStreet1 ?? undefined,
      postalCode: override?.destination?.postalCode ?? order.shipToPostalCode ?? undefined,
      areaLevel1: override?.destination?.areaLevel1 ?? order.shipToAreaLevel1 ?? undefined,
      areaLevel2: override?.destination?.areaLevel2 ?? order.shipToAreaLevel2 ?? undefined,
      areaLevel3: override?.destination?.areaLevel3 ?? order.shipToAreaLevel3 ?? undefined,
      countryCode: override?.destination?.countryCode ?? order.shipToCountryCode ?? 'MX',
    ...
    if (parcel.weight <= 0 || parcel.length <= 0 || parcel.width <= 0 || parcel.height <= 0) {
      throw new BadRequestException(
